...
DAPI_Centre_X <- subset(OnlyDAPI, Variable=='Center of Image Mass X')
DAPI_Centre_Y <- subset(OnlyDAPI, Variable=='Center of Image Mass Y')
DAPI_Centre_Z <- subset(OnlyDAPI, Variable=='Center of Image Mass Z')
...
DataW$EllipsoidAxisC_X <- abs(DataW$EllipsoidAxisC_X)
DataW$EllipsoidAxisC_Y <- abs(DataW$EllipsoidAxisC_Y)
DataW$EllipsoidAxisC_Z <- abs(DataW$EllipsoidAxisC_Z)
...
DataW <- subset(DataW, SurfaceConvolution<5)
...
fviz_eig(res.pca, addlabels = FALSE, fill = 'black') #Show scree plot (PC weightings)

fviz_contrib(res.pca, choice = "var", axes = 1, top = 10) #Show correlates of specific PCs ('axes' defines PC of interest)

quali.sup <- as.numeric(DataW[, 3]) #Set DV position as colour code for plotting PCA results
...
fviz_pca_biplot(res.pca, col.ind = quali.sup, #Plot compass plot with PCA in background
                addEllipses = FALSE, 
                alpha = 0.2, 
                label = "var", 
                repel = TRUE, 
